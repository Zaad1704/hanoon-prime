...
class JuliBrain:
    ...
    def _build_decision(
        ...
        log.info("THINK %s %s score=%.3f regime=%s risk=%s", ticker, "BUY" if direction > 0 else "SELL", score, result.get("regime", "?"), result.get("risk", "normal"))
        thought = SimpleNamespace(direction=direction, score=score, verdict=verdict, confidence=conf)
        return {"ticker": ticker, "direction": direction, "verdict": verdict, "score": score, "thought": thought, "sizing": result.get("sizing")}
    ...
    def _eval_one(
        ...
        result = self.brain.tick(alpha=self._compute_alpha(snap), ticker=ticker, entry_price=float(prices[-1]), atr=snap.get("atr", 1.0), open_positions=open_count)
