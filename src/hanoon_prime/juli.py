...
class JuliBrain:
    ...
    def tick(self, positions: set[str], get_snapshot: Any,
             streamer: Any, closing: set[str] | None = None,
    ...
    def _build_decision(self, ticker: str, direction: int,
        ...
        return {"ticker": ticker, "direction": direction, "verdict": verdict,
                "score": score, "thought": thought, "sizing": result.get("sizing")}
    def _eval_one(self, ticker: str, snap: dict[str, Any],
                  open_count: int) -> dict[str, Any] | None:
        ...
        result = self.brain.tick(
            alpha=self._compute_alpha(snap), ticker=ticker,
            entry_price=float(prices[-1]), atr=snap.get("atr", 1.0),
