...
ATTRS = (("close", "close_arr"), ("high", "high_arr"), ("low", "low_arr"),
...
class JuliBrain:
    ...
    def tick(self, positions: set[str], get_snapshot: Any, streamer: Any,
             closing: set[str] | None = None) -> tuple[list[dict[str, Any]], list[dict[str, Any]]]:
    ...
    def _sync_and_scan(self) -> None:
        ...
        try:
            if results := self.scanner.collect(): self._candidates = results
        except Exception as e:
    ...
    def _maybe_allocate(self, positions: set[str]) -> None:
        ...
        if time.time() - self._last_alloc < 5.0: return
        self._last_alloc = time.time()
        self.budget.allocate(positions, [c.symbol for c in self._candidates[:MAX_CANDIDATES]])

    def _evaluate_exits(self, positions: set[str], get_snapshot: Any, closing: set[str]) -> list[dict[str, Any]]:
        """Evaluate open positions for exit signals."""
        exits = []
        for t in positions:
            if t in closing: continue
            snap = get_snapshot(t)
            if snap is None or snap.get("last", 0) <= 0: continue
    ...
    def _evaluate_entries(self, positions: set[str], get_snapshot: Any) -> list[dict[str, Any]]:
        ...
        for ticker in self.budget.get_all_tracked() | positions:
            ...
            try:
                ...
                continue
            if dec is not None: decisions.append(dec)
        return decisions

    def _compute_alpha(self, snap: dict[str, Any]) -> dict[str, float]:
        """Compute all indicators from snapshot arrays."""
        kw = {k: [] if (v := snap.get(u)) is None else (list(v) if not isinstance(v, list) else v) for k, u in ATTRS}
        if len(kw["close"]) < 20: return {}
        alpha = compute_all_alpha(**kw)
        return alpha if alpha else (compute_alpha(**kw) or {})

    def _build_decision(self, ticker: str, direction: int, result: dict[str, Any]) -> dict[str, Any]:
        """Build decision dict from brain result."""
        score, verdict, conf = result.get("score", 0), result.get("verdict", ""), result.get("confidence", 0.5)
        log.info("THINK %s %s score=%.3f regime=%s risk=%s", ticker,
            "BUY" if direction > 0 else "SELL", score, result.get("regime", "?"), result.get("risk", "normal"))
        thought = SimpleNamespace(direction=direction, score=score, verdict=verdict, confidence=conf)
        return {"ticker": ticker, "direction": direction, "verdict": verdict, "score": score,
            "thought": thought, "sizing": result.get("sizing")}

    def _eval_one(self, ticker: str, snap: dict[str, Any], open_count: int) -> dict[str, Any] | None:
        ...
        result = self.brain.tick(alpha=self._compute_alpha(snap), ticker=ticker,
            entry_price=float(prices[-1]), atr=snap.get("atr", 1.0), open_positions=open_count)
        ...
        return self._build_decision(ticker, direction, result) if direction != 0 else None
